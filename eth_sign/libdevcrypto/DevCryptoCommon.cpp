...
#include <eth_secp256k1_ecdh.h>
#include <eth_secp256k1_recovery.h>
#include <eth_secp256k1_sha256.h>
...
Public dev::toPublic(Secret const& _secret)
{
	...
	if (eth_secp256k1_ec_pubkey_create(ctx, &rawPubkey, _secret.data()))
		return {};
...
Signature dev::sign(Secret const& _k, h256 const& _hash)
{
	auto* ctx = getCtx();
eth_secp256k1_ecdsa_recoverable_signature rawSig;
	if (!eth_secp256k1_ecdsa_sign_recoverable(ctx, &rawSig, _hash.data(), _k.data(), nullptr, nullptr))
		return {};

	Signature s;
	int v = 0;
eth_secp256k1_ecdsa_recoverable_signature_serialize_compact(ctx, s.data(), &v, &rawSig);
...
eth_secp256k1_pubkey rawPubkey;
	...
	if (eth_secp256k1_ec_pubkey_parse(ctx, &rawPubkey, serializedPubKey.data(), serializedPubKey.size()))
		return false;  // Invalid public key.
	// FIXME: We should verify the public key when constructed, maybe even keep
	//       eth_secp256k1_pubkey as the internal data of Public.
	std::array<byte, 33> compressedPoint;
	if (eth_secp256k1_ecdh_raw(ctx, compressedPoint.data(), &rawPubkey, _s.data()))
...
bytes ecies::kdf(Secret const& _z, bytes const& _s1, unsigned kdByteLen)
{
	...
	bytes k;
eth_secp256k1_sha256_t ctx;
	for (unsigned i = 0; i <= reps; i++)
	{
	eth_secp256k1_sha256_initialize(&ctx);
	eth_secp256k1_sha256_write(&ctx, ctr.data(), ctr.size());
	eth_secp256k1_sha256_write(&ctx, _z.data(), Secret::size);
	eth_secp256k1_sha256_write(&ctx, _s1.data(), _s1.size());
		// append hash to k
		std::array<byte, 32> digest;
	eth_secp256k1_sha256_finalize(&ctx, digest.data());
